...
float3 FogFilter(float3 color, float eyeDistance) {
	float fogFactor = saturate(1.0 - exp(-eyeDistance*FogExp));
...
{	
	...
	finalColor.xyz = FogFilter(finalColor.xyz, length(input.Eye));
	...
	//finalColor = float4(tangent.x, tangent.y, -tangent.z, 1.0);

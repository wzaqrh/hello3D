...
void FindBlocker(out float avgBlockerDepth, out float numBlockers, float2 uv, float z0, float2 dz_duv, float2 searchRegionRadiusUV, MIR_ARGS_TEX2D(tDepth))
{
	float blockerSum = 0;
	numBlockers = 0;
...
float PCF_Filter(float2 uv, float z0, float2 dz_duv, float2 filterRadiusUV, MIR_ARGS_SHADOWMAP(shadowMap))
{
	float sum = 0;
    ...
    for ( int i = 0; i < PCF_POISSON_COUNT; ++i )
    {
        ...
        float z = BiasedZ(z0, dz_duv, offset);
		sum += MIR_SAMPLE_SHADOW(shadowMap, float3(uv + offset, z));
	}
	...
	for (float x = -PCF_FILTER_STEP_COUNT; x <= PCF_FILTER_STEP_COUNT; ++x)
		for (float y = -PCF_FILTER_STEP_COUNT; y <= PCF_FILTER_STEP_COUNT; ++y)
		{
			float2 offset = float2(x, y) * stepUV;
			float z = BiasedZ(z0, dz_duv, offset);
			sum += MIR_SAMPLE_SHADOW(shadowMap, float3(uv + offset, z)).r;
		}            
...
float PCFShadow(float2 uv, float z, float2 dz_duv, float zEye, PCFShadowInput input, MIR_ARGS_SHADOWMAP(shadowMap), MIR_ARGS_TEX2D(shadowMapTex))
{
#if 1
	...
	FindBlocker(avgBlockerDepth, numBlockers, uv, z, dz_duv, searchRegionRadiusUV, MIR_PASS_TEX2D(shadowMapTex));
	if (numBlockers == 0) return 1.0;
#endif
	
	float2 filterRadiusUV = 0.1 * input.lightRadiusUVNearFar.xy;
	return PCF_Filter(uv, z, dz_duv, filterRadiusUV, MIR_PASS_SHADOWMAP(shadowMap));
}
...
float PCSSShadow(float2 uv, float z, float2 dz_duv, float zEye, PCFShadowInput input, MIR_ARGS_SHADOWMAP(shadowMap), MIR_ARGS_TEX2D(shadowMapTex))
{
    // ------------------------
...
inline float FastPCFShadow(float3 coord, float3 receiverPlaneDepthBias, float2 ts, MIR_ARGS_SHADOWMAP(shadowMap))
{
	float2 base_uv = coord.xy;
	float shadow = 0;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(-ts.x, -ts.y), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(0, -ts.y), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(ts.x, -ts.y), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(-ts.x, 0), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(0, 0), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(ts.x, 0), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(-ts.x, ts.y), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(0, ts.y), coord.z, receiverPlaneDepthBias)).r;
	shadow += MIR_SAMPLE_SHADOW(shadowMap, CombineShadowcoordComponents(base_uv, float2(ts.x, ts.y), coord.z, receiverPlaneDepthBias)).r;

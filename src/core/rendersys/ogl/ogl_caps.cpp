...
void OglCaps::initLimits()
{
	...
	if(check(2, 2) && VersionData.PROFILE == OglCaps::COMPATIBILITY)
	{
...
void OglCaps::initValues()
{
	...
	if(VersionData.PROFILE == OglCaps::COMPATIBILITY)
	{
		glGetFloatv(GL_POINT_SIZE_MIN, &ValuesData.POINT_SIZE_MIN);
		glGetFloatv(GL_POINT_SIZE_MAX, &ValuesData.POINT_SIZE_MAX);
	}

	glGetFloatv(GL_POINT_SIZE_RANGE, &ValuesData.POINT_SIZE_RANGE.x);
	glGetFloatv(GL_POINT_SIZE_GRANULARITY, &ValuesData.POINT_SIZE_GRANULARITY);
	glGetFloatv(GL_ALIASED_LINE_WIDTH_RANGE, &ValuesData.ALIASED_LINE_WIDTH_RANGE.x);
	glGetFloatv(GL_SMOOTH_LINE_WIDTH_RANGE, &ValuesData.SMOOTH_LINE_WIDTH_RANGE.x);
	...
	if(check(4, 1) || (ExtensionData.ARB_viewport_array))
	{
		...
		glGetFloatv(GL_VIEWPORT_BOUNDS_RANGE, &ValuesData.VIEWPORT_BOUNDS_RANGE.x);
...
OglCaps::OglCaps(Profile const & Profile) :
	VersionData(Profile),
	Version(VersionData),

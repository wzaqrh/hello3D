...
public:
	...
	const std::vector<float>& GetRawBytes() const { return mData.GetBytes(); }
...
template <> struct tpl::has_function_valid_t<UniformParameters> : public std::true_type {};
template <> struct tpl::has_function_name_t<UniformParameters> : public std::true_type {};

class GpuUniformsParameters : public tpl::Vector<UniformParameters> {
	friend class UniformParametersBuilder;
public:
